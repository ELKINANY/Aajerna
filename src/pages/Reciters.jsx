...
import { getRecitersAsync, getSingleSurahAsync } from "../redux/slices/quranSlice";
...
const Reciters = () => {
  ...
  const recitersArray = reciters ? Object.entries(reciters).map(([id, name]) => ({ 
    id, 
  ...
  const handlePlayClick = async (reciter) => {
    ...
    try {
      const response = await axios.get(`https://quranapi.pages.dev/api/audio/${surahId}.json`);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <p className="text-emerald-700 text-lg">
            {surahId && surah.surahNameArabic ? `استمع لسورة ${surah.surahNameArabic}` : "استمع إلى تلاوات عطرة من كبار القراء"}
          </p>
        </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-emerald-100 shadow-lg p-4 z-50">
            <div className="max-w-5xl mx-auto flex items-center justify-between" dir="rtl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-emerald-900">{selectedReciter?.name}</p>
                  <p className="text-sm text-emerald-600">{surah.surahNameArabic || `سورة رقم ${surahId}`}</p>
                </div>
              </div>
              <button
                onClick={() => {
